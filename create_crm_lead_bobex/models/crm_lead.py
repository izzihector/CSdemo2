...
class CrmLead(models.Model):
    ...
    def bobex_parser(self, emailBody):
        ...
        if len(a) > 1:
            ...
            descriptionindex = [i for i, s in enumerate(a) if 'Beschrijving van uw project' in s]
            if descriptionindex and descriptionindex[0]:
                plus_one = descriptionindex[0]
                final_description = a[plus_one:]
                newdict['Toelichting:'] = ''.join(final_description).replace('*', '')
        return newdict

    @api.model
    def bobex_condition(self, mailMessage, emailBody):
        """Condition for bobex template."""
        return (mailMessage.xaa_aa_company == 'bobex' or
                'bobex' in emailBody)

    @api.model
    def register_company_parser(self):
        """Register Bobex Parser Mappin in main."""
        parsers = super(CrmLead, self).register_company_parser()
        parsers.append(
            (12,
             self.bobex_condition,
             self.bobex_parser))
        return parsers
